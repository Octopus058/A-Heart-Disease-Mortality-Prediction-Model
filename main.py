...
'''
Predict the mortality of patients
'''

X = np.array(f_arr).reshape(-1, 1)
y = np.array(m_arr_scaled)
...
model = RandomForestRegressor(n_estimators=300, max_depth=30, min_samples_split=5, random_state=42)
model.fit(X_train, y_train)

y_pred = model.predict(X_test)
y_pred = np.clip(y_pred, 0, 1)

# Evaluate the stability of the model using cross validation
scores = cross_val_score(model, X_train, y_train, cv=5)
avg_score = np.mean(scores)

# Plot the actual and predicted Mortality of patients
plt.figure()
plt.plot(y_test, 'b', label='Actual Mortality(AM)')
plt.plot(y_pred, 'r', label='Predicted Mortality(PM)')

plt.title(f'The actual and predicted mortality of patients(Random forest)\nAverage cross-validation score: {avg_score:.4f}')
plt.xlabel('Subject ID')
plt.ylabel('Value')
...
y_pred_binary = ["death" if pred > 0.15 else "alive" for pred in y_pred]
y_test_binary = ["death" if actual > 0.15 else "alive" for actual in y_test]
cm = confusion_matrix(y_test_binary, y_pred_binary)

# Calculate the accuracy, recall and F1 score
accuracy = accuracy_score(y_test_binary, y_pred_binary)
recall = recall_score(y_test_binary, y_pred_binary, pos_label='death')
f1 = f1_score(y_test_binary, y_pred_binary, pos_label='death')

# Plot the confusion matrix
plt.figure(figsize=(10, 7))
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar_kws={'label': 'Number of predictions'})
plt.title('Confusion Matrix(Random Forest)\nAccuracy: {:.4f}, Recall: {:.4f}, F1: {:.4f}'.format(accuracy, recall, f1))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.show()

# Calculate the precision-recall curve & AUC
precisions, recalls, thresholds = precision_recall_curve(y_test_binary, y_pred, pos_label='death')
auc_score = auc(recalls, precisions)

plt.figure(figsize=(10,7))
plt.plot(recalls, precisions, label='AUC: {:.4f}'.format(auc_score))
...
# Reshape all data to fit the LSTM model
X_all_reshaped = np.concatenate((X_train_reshaped, X_test_reshaped))

# Create a KerasRegressor instance for cross validation
model_cv = KerasRegressor(build_fn=create_model, epochs=300, batch_size=10, verbose=0)

# Evaluate the stability of the model using cross validation
scores = cross_val_score(model_cv, X_all_reshaped, np.concatenate((y_train, y_test)), cv=5)
avg_score = np.mean(scores)
...
plt.title(f'The actual and predicted Mortality of patients(LSTM)\nAverage cross-validation score: {avg_score:.4f}')
plt.xlabel('Subject ID')
plt.ylabel('Value')
